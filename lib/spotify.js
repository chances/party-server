...
export function playlists (opts, callback) {
  ...
  getAuth(query, opts.token, callback);
}

export function playlist (opts, callback) {
  if (!opts.userId || !opts.playlistId || !opts.token) {
    throw new Error('userId, playlistId, and token are required');
  }
  let query = '/v1/users/' + opts.userId + '/playlists/' + opts.playlistId;
  getAuth(query, opts.token, callback);
}
